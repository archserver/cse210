...
class Program
{
    static void Main(string[] args)
    {
        ...
        // the entire stored reference of the book, chapter and verse(s)
        Reference reference;
        // the requested scripture in text broken down into words
        Scripture scripture;
        // the temporary holding locationd of the book, chapter and verses
...
//        List<Book> books = ReadBookfromJsonFormat(filename); - Issue with auto Parsing content to structure
        ...
        // printing out the verses to be used
/*        for (int i = verseSelectionLow; i <= verseSelectionHigh; i++)
        ...
        // the entire stored reference of the book, chapter and verse(s)
        if (verseSelectionLow == verseSelectionHigh)
        {
            reference = new Reference(bookSelection, chapterSelection, verseSelectionLow);
        }
        else
        {
            reference = new Reference(bookSelection, chapterSelection, verseSelectionLow, verseSelectionHigh);
        }

        scripture = new Scripture(reference.GetDisplayText(books));

        do
        {
            scripture.PrintTheVerse(reference);
            ...
            if (entry == "")
            {
                finished = scripture.HideSomeWords();
            }
            else if (entry == "Quit")
            {
                finished = true;
